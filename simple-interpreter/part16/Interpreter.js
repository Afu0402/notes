...
const WRONG_PARAMS_NUM = 'Wrong number of arguments';
...
class SemanticAnalyzer extends NodeVisitor {
  ...
  visit_ProcedureCall(node) {
    ...
    const proc_symbol = this.current_scope.lookup(proc_name);
    if(proc_symbol.params.length !== node.actual_params.length) {
      this.error(WRONG_PARAMS_NUM,node.token)
    }
